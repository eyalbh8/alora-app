...
const builtFramesBySlide = new Map();
let activeBuildPage = null;
let previousBuildPage = null;
...
figma.ui.onmessage = async (msg) => {
    if (msg.type === 'apply-qa') {
        try {
            applyQaCorrections(msg.results);
    ...
    try {
        ...
        if (activeBuildPage) {
            if (previousBuildPage)
                await figma.setCurrentPageAsync(previousBuildPage);
...
async function buildCarousel(build) {
    ...
    for (let i = 0; i < slideCount; i++) {
        ...
        if (Array.isArray(resolvedLayout.layers)) {
            for (const textLayer of resolvedLayout.layers) {
                try {
                    const result = await addTextLayer(frame, {
                        ...textLayer,
...
function addScrimOverlay(frame, overlay, dimensions) {
...
async function addLogoLayer(frame, base64, logo, dimensions) {
...
function resolveSlideTextLayout(slide, step6Image, dimensions = INSTAGRAM_DIMENSIONS) {
    ...
    const layers = (slide.textLayers || []).map((layer) => ({
        ...layer,
        color: step6Image?.resolvedTextBlock?.source === 'profile-scrim'
            ? layer.color
...
function fitTextStack(nodes, layers, bounds) {
    if (!nodes.length)
        return bounds;
    const padding = 40;
    const gap = 24;
    ...
    for (let scale = 1; scale >= minimumScale - 0.001; scale -= 0.04) {
        for (let index = 0; index < nodes.length; index++) {
            nodes[index].fontSize = Math.max(Number(layers[index].typography?.minFontSize) || 12, initialSizes[index] * scale);
...
async function addTextLayer(frame, textLayer, slideIndex) {
    const weightStyles = {
        '400': 'Regular',
        '500': 'Medium',
        '600': 'SemiBold',
    ...
    const segments = Array.isArray(textLayer.typography?.segments)
        ...
        : [];
    if (!fontSubstituted && segments.some((segment) => segment.italic)) {
        const italicName = {
            family: requestedFamily,
...
function applyQaCorrections(results) {
    for (const result of results) {
        const frame = builtFramesBySlide.get(result.slideIndex);
        if (!frame)
            continue;
        for (const correction of result.corrections || []) {
            const node = frame.findOne((candidate) => candidate.type === 'TEXT' && candidate.name === correction.layerId);
            if (!node)
                continue;
        ...
        if (result.panel?.required &&
            ...
            if (textNodes.length) {
                ...
                addEditableBackingPanel(frame, { x: left, y: top, width: right - left, height: bottom - top }, {
...
function addEditableBackingPanel(frame, bounds, panelSpec) {
...
function validateTextNodes(nodes, blueprints, slideIndex, dimensions = INSTAGRAM_DIMENSIONS) {
    if (!nodes.length)
        return;
...
function addDebugZoneVisualization(frame, zones, _slideIndex) {
